...
import { FullCalendarDemoComponent } from 'app/primeng/data/fullcalendar';
declare var $: any;
...
export class MissionComponent implements OnInit, OnDestroy {
  ...
  events: any[];
  options: any;
  @ViewChild('calendar', null) calendarComponent: FullCalendarDemoComponent;
      ...
      .subscribe(
        (res: HttpResponse<IMission[]>) => {
          res.body.map(m => {
            this.events.push({
              title: m.team['name'],
              start: m.startDate['_i']
            });
          });
          this.calendarComponent.events = this.events;
          this.paginateMissions(res.body, res.headers);
        },
